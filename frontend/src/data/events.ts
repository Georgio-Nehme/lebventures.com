// src/data/events.ts
// When PUBLIC_API_URL is set, events are fetched from the live API.
// Falls back to demo data so the site still builds without a backend.
...
export interface Event {
  ...
  image?: string;      // URL to event cover photo
  trailFile?: string;  // URL to GPX/KML route file
}

// ---------------------------------------------------------------------------
// Fetches events from the live API when PUBLIC_API_URL is configured,
// otherwise returns the demo data below.
// ---------------------------------------------------------------------------
export async function getEvents(): Promise<Event[]> {
  ...
  if (apiUrl) {
    try {
      ...
      console.warn('API fetch failed, falling back to demo data');
    } catch (err) {
      console.warn('API unreachable, falling back to demo data:', err);
    }
  }

  // Demo data — remove once backend is live
  return [
    {
      id: 'ev-001',
